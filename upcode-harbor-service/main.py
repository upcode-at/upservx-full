...
apply_secure_umask()
PASSIVE_PROCESS = os.getenv("UPCODE_HARBOR_PASSIVE_PROCESS") == "1"

# ---------------------------------------------------------------------------
# Logging – tee stdout/stderr to log file
# ---------------------------------------------------------------------------

LOG_FILE = os.getenv("UPCODE_HARBOR_LOG_FILE", "/etc/upcode-harbor.log")
...
if __name__ == "__main__":
    ...
    try:
        ...
        uvicorn.run(
            app,
            host=os.getenv("UPCODE_HARBOR_API_HOST", "0.0.0.0"),
            port=9500,
            workers=1,
        )
    finally:
