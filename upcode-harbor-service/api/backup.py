...
async def test_backup_server(server_id: int):
    ...
    try:
        ...
        if server["type"] == "local":
            local_path = server.get("local_path") or "/var/lib/upcode-harbor/backups"
            ...
            if success:
                test_path = posixpath.join(
                    storage.remote_path,
                    f".upcode-harbor-write-test-{uuid.uuid4().hex}",
                )
                try:
...
async def get_backup_server_info(server_id: int):
    ...
    if not server:
        raise HTTPException(status_code=404, detail="Backup server not found")
    if server["type"] == "local":
        path = server.get("local_path") or "/var/lib/upcode-harbor/backups"
