...
SESSION_SECRET_FILE = os.getenv(
    "UPCODE_HARBOR_SESSION_SECRET_FILE",
    "/etc/upcode-harbor/.session_secret",
)
SESSION_STORE_FILE = os.getenv(
    "UPCODE_HARBOR_SESSION_STORE_FILE",
    "/etc/upcode-harbor/sessions.json",
)
SESSION_LOCK_FILE = os.getenv(
    "UPCODE_HARBOR_SESSION_LOCK_FILE",
    "/etc/upcode-harbor/sessions.lock",
)
...
def _get_secret() -> bytes:
    ...
    env_secret = os.getenv("UPCODE_HARBOR_SESSION_SECRET", "").strip()
    if env_secret:
        if len(env_secret.encode("utf-8")) < 32:
            raise SessionStoreError("UPCODE_HARBOR_SESSION_SECRET must contain at least 32 bytes")
