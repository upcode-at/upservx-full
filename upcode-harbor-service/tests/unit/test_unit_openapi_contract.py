...
def test_generated_clients_do_not_put_secrets_in_query_strings():
    ...
    client = (root / "upcode-harbor" / "lib" / "api.ts").read_text()
...
def test_cli_backup_payload_uses_generated_contract_type():
    root = Path(__file__).resolve().parents[3]
    command = (root / "upcode-harbor-cli" / "cli" / "commands" / "backup.py").read_text()
...
def test_generated_request_types_match_openapi(app):
    ...
    assert (root / "upcode-harbor" / "lib" / "generated-api-types.ts").read_text() == typescript
    assert (root / "upcode-harbor-cli" / "cli" / "generated_api_types.py").read_text() == python


def test_incomplete_cluster_sync_is_not_exposed_as_working():
    root = Path(__file__).resolve().parents[3]
    sync_manager = (root / "upcode-harbor-service" / "lib" / "container_sync.py").read_text()
    workload_ui = (root / "upcode-harbor" / "components" / "workload-distribution.tsx").read_text()
