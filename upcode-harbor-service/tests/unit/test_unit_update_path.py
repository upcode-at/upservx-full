...
def _load_updater():
    path = ROOT / "deploy" / "upcode-harbor-updater"
    spec = importlib.util.spec_from_loader(
        "upcode_harbor_updater",
        SourceFileLoader("upcode_harbor_updater", str(path)),
    )
...
def test_incomplete_signed_release_is_rejected(tmp_path):
    updater = _load_updater()
    (tmp_path / "upcode-harbor").mkdir()
    (tmp_path / "upcode-harbor" / "package-lock.json").write_text("{}")
...
def test_release_manifest_must_match_requested_version(tmp_path):
    updater = _load_updater()
    required = (
        tmp_path / "upcode-harbor/package-lock.json",
        tmp_path / "upcode-harbor-service/requirements.lock",
        tmp_path / "upcode-harbor-service/main.py",
        tmp_path / "upcode-harbor/public/novnc/vnc.html",
    )
    for path in required:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("content")
    (tmp_path / ".upcode-harbor-release.json").write_text(
...
def test_nonzero_external_updater_result_fails_the_job(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("UPCODE_HARBOR_UPDATE_STATE_ROOT", str(tmp_path))
