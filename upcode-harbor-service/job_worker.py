...
logger = logging.getLogger("upcode-harbor.job-worker")
POLL_SECONDS = float(os.getenv("UPCODE_HARBOR_JOB_POLL_SECONDS", "1"))
HEARTBEAT_SECONDS = float(os.getenv("UPCODE_HARBOR_JOB_HEARTBEAT_SECONDS", "5"))
WORKER_LOCK = Path(os.getenv("UPCODE_HARBOR_JOB_WORKER_LOCK", str(CONFIG_ROOT / "job-worker.lock")))
    ...
    def run_forever(self) -> None:
        ...
        while not self.stopping:
            ...
            installed_runner = Path(
                "/opt/upcode-harbor/current/upcode-harbor-service/job_worker.py"
            )
