...
STATE_DIR = os.getenv("UPCODE_HARBOR_STATE_DIR", "/var/lib/upcode-harbor")
SSH_KEY_DIR = os.getenv("UPCODE_HARBOR_SSH_KEY_DIR", os.path.join(STATE_DIR, "ssh_keys"))
AUTHORIZED_KEYS_DIR = os.getenv(
    "UPCODE_HARBOR_AUTHORIZED_KEYS_DIR", os.path.join(STATE_DIR, "authorized_keys")
)
...
class SSHKeyManager:
    ...
    def generate_ssh_key_pair(self, 
        ...
        try:
            ...
            public_key_content = f"{public_ssh.decode()} {key_name}@upcode-harbor"
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    public_key_content = f"{result.stdout.strip()} {key_name}@upcode-harbor"
                    secure_write_text(public_key_path, public_key_content)
                    os.chmod(public_key_path, 0o644)
                else:
