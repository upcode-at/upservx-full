...
class LocalBackupStorage(BackupStorage):
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        self.backup_path = config.get('path', '/var/lib/upcode-harbor/backups')
...
class BackupManager:
    ...
    def verify_archive(
        ...
        *,
        ...
        validation_dir = tempfile.mkdtemp(prefix="upcode-harbor-verify-")
        try:
            with tarfile.open(archive_path, "r:*") as archive:
        ...
        if test_restore:
            test_dir = tempfile.mkdtemp(prefix="upcode-harbor-test-restore-")
            try:
    ...
    def _validated_instance_path(
        ...
        if server["type"] == "local":
            ...
            roots = (
                [configured_root]
                if configured_root
                else ["/var/lib/upcode-harbor/backups", "/tmp/backups"]
            )
    ...
    def _local_archive(self, instance: Dict[str, Any], server: Dict[str, Any]):
        ...
        temp_dir = tempfile.mkdtemp(prefix="upcode-harbor-restore-")
    ...
    def _consistent_vm_disks(self, vm_name: str):
        ...
        snapshotted = False
        if running:
            snapshot_name = f"upcode-harbor-backup-{uuid.uuid4().hex[:12]}"
    ...
    def execute_backup(
        ...
        try:
            ...
            if server['type'] == 'local':
                dest_dir = server.get('local_path') or '/var/lib/upcode-harbor/backups'
            ...
            with tarfile.open(backup_path, mode, **tar_kwargs) as tar:
                ...
                for i, target in enumerate(job['targets']):
                    ...
                    if target.startswith('vm:'):
                        ...
                        export_dir = tempfile.mkdtemp(prefix="upcode-harbor-container-")
                        try:
