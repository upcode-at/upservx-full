...
def test_read_log_file_supports_nested_relative_paths(tmp_path, monkeypatch):
    ...
    activity_log = log_root / "upservx" / "activity.log"
    ...
    assert settings.read_log_file("upservx/activity.log", lines=0) == "nested log entry\n"
...
def test_get_log_files_includes_nested_activity_log(tmp_path, monkeypatch):
    ...
    assert "upservx/activity.log" in names